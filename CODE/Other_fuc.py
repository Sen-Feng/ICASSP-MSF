...
def juli_gailvhua(lab,dataset,sim):
    ...
    for k in range(K):
        ...
        indexk.append(indexk)
        attribute = []
        number = []
        for d in range(D):
            av , vn = np.unique(dataset[index_k, d], return_counts=True)
            attribute.append(av.tolist())
            number.append(vn.tolist())
        attribute_value.append(attribute)
        value_number.append(number)
    # print(attribute_value)

    for i in range(N):
        for k in range(K):  # 对于每个簇 计算当前点与簇的关系
            for d in range(D):  # 对于每个属性
                av = np.array(attribute_value[k][d])
                # index = np.where(attribute_value[k][d] == dataset[i, d])[0]
                index = np.where(av == dataset[i, d])[0]
                # print(index)
                if len(index) != 0:
                    distance[i, k] += value_number[k][d][index[0]] / len(indexk[k])
